...
export async function POST(request: NextRequest) {
  try {
    ...
    const employeeId = Number(body.employee_id || body.id || 0);
    const fullName = normalizeText(
      body.full_name || body.nom_complet || body.employeeName || body.employeName
    );

    const oldPassword = normalizeText(body.old_password || body.oldPassword);
    const newPassword = normalizeText(body.new_password || body.newPassword);
    ...
    if (newPassword.length < 4) {
      return NextResponse.json(
        {
          success: false,
          message: "Le nouveau mot de passe doit contenir au moins 4 caracteres.",
        },
        { status: 400 }
      );
    }
    ...
    const oldPasswordOk = await bcrypt.compare(oldPassword, passwordHash);
    ...
    db.prepare(
      ...
      WHERE id = ?
      `
    ).run(newPasswordHash, now, employee.id);
