...
function getRequestUser(item?: RequestItem) {
  ...
  return (
    ...
    cleanText(item.utilisateur) ||
    cleanText(item.employee_name) ||
    cleanText(item.active_user_name)
  );
}
...
export default function ResponsableDashboardPage() {
  ...
  const currentActiveUser = useMemo(() => {
    ...
    if (
      fromDashboard &&
      !normalize(fromDashboard).includes("utilisateur actuellement")
    ) {
      return fromDashboard;
    }

    const lastAuthorized = authorizedRequests[0];
    const fromAuthorized = getRequestUser(lastAuthorized);

    if (fromAuthorized) return fromAuthorized;

    const anyNamedRequest = requests.find((item) => getRequestUser(item));
    const fromAnyRequest = getRequestUser(anyNamedRequest);

    if (fromAnyRequest) return fromAnyRequest;

    return "Utilisateur non identifie";
  }, [dashboard, requests, authorizedRequests]);
  ...
  async function loadDashboard(showLoader = false) {
    try {
      ...
      const [dashboardRes, requestsRes, historyRes] = await Promise.all([
        ...
        fetch("/api/history?page=1&pageSize=10&sort=recent", {
          cache: "no-store",
        }),
      ]);
